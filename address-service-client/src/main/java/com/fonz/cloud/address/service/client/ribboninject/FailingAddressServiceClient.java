...
//    private ServerList<Server> ribbonServerList;
    ...
    public String getAddress() throws RestClientException, IOException {
        ...
        // WARNING! - This will not yield the proper result on CF.
        // Injecting a CF routing header on application level will not include the 
        // header in retry requests issued by Ribbon. And hence Ribbon will not have any control
        // over the srevice instance it sends the request to - it will be overruled by the CF Go-Router - or worse:
        // if this header is set on application level, ribbon will always route to the same single instance.
        // 
        // Solution: Don't do it on application level. Ribbon retry requests will always include it
        // and never get a chance to route to the instance they actually target.
        // Do it with a LoadBalancerRequestTransformer see CFLaodBalancerRequestTransformer in package 
        // com.fonz.cloud.address.service.client.cf

        
        // Inject Routing Header on application level:
